...
@shared_task(bind=True)
def send_trial_cancellation_mail(self,recipient_list,trial_name,academy_name):
    try:
        print(' incelery to send cancellation mail',academy_name)
        subject = 'Trail cancelled'
        message = f"Dear player, \n\n we regret to inform you that the trial '{trial_name}' conducted by '{academy_name}' has been cancelled. Any payment processed will be refunded wihtin 3-7 days . \n Contact {academy_name} for further information. \n\n Best regards , \n PlayMaker"
        email_from = settings.EMAIL_HOST_USER
